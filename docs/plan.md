...
- [x] In-app auto-update (0.6.4).
  - `tauri-plugin-updater` + `tauri-plugin-process` behind
    `src/lib/adapters/updater/`; the browser build reports the feature as
    unsupported and keeps using the service worker.
  - Background check after launch and every six hours, a bottom prompt for the
    hit, Settings ▸ Updates and Calqo ▸ Check for Updates… for on-demand runs,
    plus an auto-check switch and a per-version skip.
  - Signed `latest.json` served from the GitHub release; the public key ships in
    `src-tauri/tauri.conf.json`. Setup and release routine in
    `docs/releasing.md`.
- [x] Automate the release build (0.6.4).
  - `.github/workflows/release.yml` builds on an Apple Silicon runner and fails
    the run if AVFoundation/VideoToolbox is missing from the binary, if the
    updater signature is absent, or if the public key is unset.
  - `pnpm version:check` keeps `package.json`, `tauri.conf.json`, and
    `Cargo.toml` in agreement; the workflow also checks them against the tag.
  - The release is drafted, not published, so publishing stays a deliberate act.
  - Typecheck/lint/test stay local (`pnpm release:check`) rather than burning
    macOS runner minutes at 10x billing.
- [ ] Add the remaining release artifact checks.
  - SHA-256 generation.
  - Third-party notices if needed.
  - License included.
  - (Done: app version visible in UI — About and Settings ▸ Updates; release
