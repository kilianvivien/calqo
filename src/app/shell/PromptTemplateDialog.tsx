...
export function PromptTemplateDialog({
  ...
  return (
    <PromptTemplateDialogInner onOpenAgentSettings={onOpenAgentSettings} />
  );
}
      ...
      <div className="space-y-4">
        ...
        <Field label={t('promptTemplate.format')}>
          <select
            ...
            value={preset}
            onChange={(event) =>
              setPreset(event.target.value as ArtboardPresetId)
            }
          ...
          >
            {ARTBOARD_PRESET_LIST.map((p) => (
              <option key={p.id} value={p.id}>
                {t(`presets.${p.id}`, { defaultValue: p.name })} ({p.width}×
                {p.height})
              </option>
            ))}
          </select>
          ...
          >
            {COMMON_CONTENT_LOCALES.map((l) => (
              <option key={l.code} value={l.code}>
                {localeLabel(l.code, i18n.language)} ({l.code})
              </option>
            ))}
          </select>
        </Field>
          ...
          <div className="rounded-[var(--calqo-radius-sm)] border border-[#FF5F57]/40 bg-[#FF5F57]/10 p-3">
            ...
            {failure.diagnostics && (
              <dl className="mt-2 grid grid-cols-[76px_1fr] gap-x-2 gap-y-1 rounded-[var(--calqo-radius-sm)] bg-[var(--calqo-glass-thin)] p-2 text-[10.5px]">
                <dt className="text-[var(--calqo-text-3)]">
                  {t('promptTemplate.provider')}
                </dt>
      ...
      <footer className="mt-5 flex items-center gap-2">
        ...
        <div className="ml-auto flex items-center gap-2">
          ...
          >
            ...
            {busy
              ? t('promptTemplate.generating')
              : t('promptTemplate.generate')}
          </GlassButton>
        </div>
      </footer>
    </ModalOverlay>
